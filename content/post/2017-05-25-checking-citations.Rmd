...
* My current approach is to copy and paste the contents of a manuscript (a Word document) into a text file (removing all images), then using `xCitations()` on the text file. 
    * This works well for a Word document, but being able to read **the Word file directly** would be an improvement.
    * This does not work as well for manuscripts in PDF files, because citations that are wrapped across multiple lines would be missed.  Being able to **read the PDF file directly** would be an improvement.
